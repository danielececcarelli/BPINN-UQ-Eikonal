...
"""
Add the path to the data from pykonal"""
sys.path.append("data")

"""
load the analytical functions for datasets: exponential, circle, anisotropic1 and anisotropic2
"""
from exponential.analytical_functions import ATexact as ATexp
from exponential.analytical_functions import CVexact as CVexp

from circle.analytical_functions import ATexact as ATcircle
from circle.analytical_functions import CVexact as CVcircle

from anisotropic1.analytical_functions import ATexact as ATanis1
from anisotropic1.analytical_functions import CVexact as CVanis1

from anisotropic2.analytical_functions import ATexact as ATanis2
from anisotropic2.analytical_functions import CVexact as CVanis2
...
class dataset_class:
    ...
    def _build_analytical_dataset(self):
        ...
        if(self.n_input == 1):
            self.n_1 = self.n_domain
        if(self.n_input == 2):
            n = math.floor(math.sqrt(self.n_domain)) # TODO: problem when n_collocation not a perfet square or cube!
                                                          # n_other? dove li devo mettere? reshape solo sui primi n**2 ?
            self.n_1 = self.n_2 = n
        if(self.n_input == 3):
    ...
    def build_noisy_dataset(self):
        ...
        if(self._flag_dataset_noise == False):  # we add the noise only the first time
            ...
            for i in range(0,len(self.T_exact)):
                ...
                #v_error = v_error*np.abs(self.CV_exact[i,:])
    ...
    def get_axis_data(self):
        ...
        if(self.dataset_type == "analytical"):
            ...
            inputs = self.inputs_dom[::n, :]    # TODO: quando non è un quadrato/cubo? usare n_1,n_2,n_3
            T = self.T_dom[::n, :]
            CV = self.CV_dom[::n, :]
    ...
    def plot(self, save_path = "plot"):
        ...
        if(self._flag_dataset_noise == False):
            self.build_noisy_dataset()
        if(self.n_input == 1):
            self.plot_1D(save_path) # 1D plot
        elif(self.n_input == 2):
            if(self.pde_type == "anisotropic"):
                self.plot_2D_anisotropic(save_path) # 2D plot for anisotropic case
            else:
                self.plot_2D(save_path) # 2D plot
        elif(self.n_input == 3):
            self.plot_3D(save_path) # 3D plot
        else:
            raise Exception("Only case 1D, 2D, 3D allowed")


    def plot_1D(self, save_path = "plot"):
        """plot the 1D example"""
        x,at,v = self.get_dom_data()
        x_e,at_e,v_e = self.get_exact_data()
        fig = plt.figure()
        fig.set_size_inches((12,5))
        plt.subplot(1,2,1)
        plt.plot(x, at, label = 'at')
        plt.plot(x_e, at_e, 'r*')
        plt.subplot(1,2,2)
    ...
    def plot_2D(self, save_path = "plot"):
        """plot the 2D example"""
        inputs,at,v = self.get_dom_data()
    ...
    def plot_2D_anisotropic(self, save_path = "plot/"):
        """plot the 2D anisotropic example"""
        if(self._flag_dataset_build == False):
            self.build_dataset()
        ...
        x = inputs[:,0]
        y = inputs[:,1]
        inputs_e,_,_ = self.get_exact_data()
        x_e = inputs_e[:,0]
        y_e = inputs_e[:,1]
        ...
        for i in range(v.shape[1]):
            plt.subplot(2,2, i+2)
            plt.scatter(x, y, c=v[:,i], label = 'at', cmap = 'coolwarm', vmin = min(v[:,i]), vmax = max(v[:,i]))
            plt.colorbar()
            plt.xlim([0,1])
            plt.ylim([0,1])
            plt.title('exact '+letter[i])
        #plt.tight_layout()
    ...
    def plot_domain(self, save_path = "plot/"): # do we need it?
