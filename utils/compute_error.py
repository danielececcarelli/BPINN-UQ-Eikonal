...
class compute_error:
    ...
    def error(self):
        ...
        elif self.n_output_vel == 0: # scalar Isotropic
            # get domain data
            inputs,at_true,_ = self.datasets_class.get_dom_data()
            at_true = at_true.squeeze()
            v_true = 1.#v_true.squeeze()

            # compute output_times
            output_times = []
            for i in range(self.bayes_nn.num_neural_networks):
                output_times.append(self.bayes_nn.nnets_times[i].features(inputs))
            output_times = tf.squeeze(tf.stack(output_times))

            np.save(os.path.join(self.path_result,"v_final.npy"),self.bayes_nn.log_betas.v_scalar.numpy())

            at_NN = tf.math.reduce_mean(output_times, axis=0).numpy()
            at_std = tf.math.reduce_std(output_times, axis=0).numpy()

            sigma = 0.
            if(self.bayes_nn.log_betas.betas_trainable_flag()):
                for log_b in self.bayes_nn.log_betas.get_trainable_log_betas()[:-1]:
                    b = np.exp(log_b.numpy())
                    s = np.sqrt(np.reciprocal(b))
                    sigma += np.mean(s)
            at_std += sigma

            v_NN = tf.math.reduce_mean(self.bayes_nn.log_betas.v_scalar).numpy()
            v_std = tf.math.reduce_std(self.bayes_nn.log_betas.v_scalar).numpy()
            v_std+=sigma

            print("v estimated: ", self.bayes_nn.log_betas.v_scalar.numpy())


            ### to compute the MSError
            loss_f = tf.keras.losses

            # accruacy of at -> MSE fill_between at_NN and at_true
            error_at = loss_f.MSE(at_NN, at_true).numpy()

            # accuracy of v -> MSE fill_between v_NN and v_true
            error_v = (v_NN-v_true)**2
